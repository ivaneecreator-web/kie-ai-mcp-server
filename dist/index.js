...
import { CallToolRequestSchema, ErrorCode, ListToolsRequestSchema, ListResourcesRequestSchema, ReadResourceRequestSchema, ListPromptsRequestSchema, GetPromptRequestSchema, McpError, } from "@modelcontextprotocol/sdk/types.js";
...
class KieAiMcpServer {
    ...
    static ALL_TOOLS = Array.from(new Set([
    ...
    constructor() {
        ...
        this.config = {
            ...
            callbackUrlFallback: process.env.KIE_AI_CALLBACK_URL_FALLBACK ||
    ...
    validateToolNames(tools) {
        ...
        if (invalidTools.length > 0) {
            throw new Error(`Invalid tool names: ${invalidTools.join(", ")}. ` +
    ...
    getEnabledTools() {
        ...
        if (disabledToolsEnv) {
            ...
            const disabledUtilityTools = disabledTools.filter((t) => KieAiMcpServer.TOOL_CATEGORIES.utility.includes(t));
            ...
            const nonUtilityDisabled = disabledTools.filter((t) => !KieAiMcpServer.TOOL_CATEGORIES.utility.includes(t));
    ...
    formatError(toolName, error, paramDescriptions) {
        ...
        if (error instanceof Error) {
            ...
            if (errorMessage.includes("ZodError")) {
                ...
                const validationErrors = lines.filter((line) => line.includes("Expected") ||
    ...
    setupHandlers() {
            ...
            const allTools = [
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            prompt: {
                                type: "string",
                                description: "Text prompt for image generation or editing (max 20000 chars). Nano Banana models support up to 20K characters.",
                                minLength: 1,
                                maxLength: 20000,
                            },
                            image_input: {
                                type: "array",
                                description: "Array of reference image URLs for editing mode (up to 14 images for multi-reference)",
                            ...
                            resolution: {
                                type: "string",
                                enum: ["1K", "2K", "4K"],
                                description: "Output resolution: 1K (8 credits), 2K (12 credits), 4K (18 credits)",
                                default: "1K",
                            },
                            google_search: {
                                type: "boolean",
                                description: "Enable Google Search grounding for factual image generation",
                                default: false,
                            },
                        },
                        required: [],
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            imageUrls: {
                                type: "array",
                                description: "Image URLs for image-to-video generation: 1 image (video unfolds around it) or 2 images (first=start frame, second=end frame)",
                                items: { type: "string", format: "uri" },
                                maxItems: 2,
                                minItems: 1,
                            },
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            prompt: {
                                type: "string",
                                description: "Text prompt for image generation or editing. V4: max 5000 chars, V5 Lite: max 3000 chars (API returns 500 error if exceeded)",
                                minLength: 1,
                                maxLength: 5000,
                            },
                            image_urls: {
                                type: "array",
                                description: "Array of image URLs for editing mode (optional - if not provided, uses text-to-image). V4: max 10, V4.5: max 14",
                                items: {
                                    type: "string",
                                    format: "uri",
                                },
                            ...
                            version: {
                                type: "string",
                                description: "Seedream version: '4' for V4, '5-lite' for V5 Lite (default) with enhanced features",
                                enum: ["4", "5-lite"],
                                default: "5-lite",
                            },
                            image_size: {
                            ...
                            seed: {
                                type: "integer",
                                description: "Random seed for reproducible results (V4 only, use -1 for random)",
                                default: -1,
                            },
                            aspect_ratio: {
                                type: "string",
                            ...
                            quality: {
                                type: "string",
                                description: "Output quality for V5 Lite (V5 Lite only): 'basic' = 2K, 'high' = 3K resolution",
                                enum: ["basic", "high"],
                                default: "basic",
                            },
                            callBackUrl: {
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            prompt: {
                                type: "string",
                                description: "Text prompt describing the desired image (max 5000 characters). Supports bilingual prompts.",
                                minLength: 1,
                                maxLength: 5000,
                            },
                            aspect_ratio: {
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            prompt: {
                                type: "string",
                                description: "Text prompt describing the desired content (required for text modes, optional for image-to-video)",
                                maxLength: 5000,
                            },
                            image_urls: {
                                type: "array",
                                description: "Single image URL for image-to-video mode (alternative to task_id)",
                                items: { type: "string", format: "uri" },
                                maxItems: 1,
                            },
                            task_id: {
                                type: "string",
                                description: "Task ID from a previous Grok generation (for upscale or image-to-video from generated image)",
                            },
                            index: {
                                type: "integer",
                                description: "Image index from task_id (0-5, Grok generates 6 images per task)",
                                minimum: 0,
                                maximum: 5,
                            },
                            aspect_ratio: {
                            ...
                            mode: {
                                type: "string",
                                description: "Generation style: 'normal' (default), 'fun' (playful), 'spicy' (expressive, not available with external images)",
                                enum: ["fun", "normal", "spicy"],
                                default: "normal",
                            },
                            generation_mode: {
                                type: "string",
                                description: "Explicit mode selection (auto-detected if not provided): text-to-image, text-to-video, image-to-video, or upscale",
                                enum: [
                                    "text-to-image",
                                    "text-to-video",
                                    "image-to-video",
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            image_url: {
                                type: "string",
                                description: "URL of the portrait image to animate (JPEG, PNG, WEBP, max 10MB)",
                                format: "uri",
                            },
                            audio_url: {
                                type: "string",
                                description: "URL of the audio file for lip sync (MPEG, WAV, AAC, MP4, OGG, max 10MB)",
                                format: "uri",
                            },
                            prompt: {
                                type: "string",
                                description: "Text prompt to guide video generation (e.g., 'A young woman talking on a podcast')",
                                minLength: 1,
                                maxLength: 1500,
                            },
                            resolution: {
                                type: "string",
                                description: "Video resolution: 480p (faster, cheaper) or 720p (higher quality)",
                                enum: ["480p", "720p"],
                                default: "480p",
                            },
                            seed: {
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            image_url: {
                                type: "string",
                                description: "URL of the portrait image for avatar (JPEG, PNG, WEBP, max 10MB)",
                                format: "uri",
                            },
                            audio_url: {
                                type: "string",
                                description: "URL of the audio file for the avatar to speak (MPEG, WAV, AAC, MP4, OGG, max 10MB)",
                                format: "uri",
                            },
                            prompt: {
                                type: "string",
                                description: "Text prompt to guide video generation (emotions, expressions, scene settings)",
                                minLength: 1,
                                maxLength: 1500,
                            },
                            quality: {
                                type: "string",
                                description: "Video quality: standard (720P, faster) or pro (1080P, higher quality)",
                                enum: ["standard", "pro"],
                                default: "standard",
                            },
                            callBackUrl: {
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            prompt: {
                                type: "string",
                                description: "Text prompt describing the desired video content (max 5000 characters). For audio: use [Character name, voice style] format for dialogue",
                                minLength: 1,
                                maxLength: 5000,
                            },
                            image_urls: {
                                type: "array",
                                description: "Up to 2 image URLs: first = start frame, second = end frame (optional - if not provided, uses text-to-video)",
                                items: { type: "string", format: "uri" },
                                maxItems: 2,
                            },
                            duration: {
                            ...
                            mode: {
                                type: "string",
                                description: "Quality mode: 'std' for standard (faster, cheaper), 'pro' for professional quality",
                                enum: ["std", "pro"],
                                default: "std",
                            },
                            sound: {
                                type: "boolean",
                                description: "Enable native audio generation including multilingual speech, sound effects, and ambient sound. Pricing: with audio is 2x credits",
                                default: false,
                            },
                            multi_shots: {
                                type: "boolean",
                                description: "Enable multi-shot mode for cinematic storytelling with multiple scenes (requires multi_prompt)",
                                default: false,
                            },
                            multi_prompt: {
                                type: "array",
                                description: "Array of shot definitions for multi-shot mode. Each shot has a prompt and duration (1-12s)",
                                items: {
                                    type: "object",
                                    properties: {
                                        prompt: {
                            ...
                            kling_elements: {
                                type: "array",
                                description: "Character/object elements for consistent identity across shots. Provide name, description, and reference images/videos",
                                items: {
                                    type: "object",
                                    properties: {
                                        name: {
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            prompt: {
                                type: "string",
                                description: "Text prompt describing the desired video content (max 1500 characters)",
                                minLength: 1,
                                maxLength: 1500,
                            },
                            imageUrl: {
                                type: "string",
                                description: "URL of input image for image-to-video mode (optional - if not provided, uses text-to-video)",
                                format: "uri",
                            },
                            endImageUrl: {
                                type: "string",
                                description: "URL of end frame image for image-to-video (optional - requires imageUrl)",
                                format: "uri",
                            },
                            version: {
                                type: "string",
                                description: "Hailuo model version: '02' (original) or '2.3' (better motion, expressions, 1080P support)",
                                enum: ["02", "2.3"],
                                default: "02",
                            },
                            quality: {
                                type: "string",
                                description: "Quality level of video generation (standard for faster, pro for higher quality)",
                                enum: ["standard", "pro"],
                                default: "standard",
                            },
                            duration: {
                                type: "string",
                                description: "Duration of video in seconds (standard quality only). Note: 10s not supported with 1080P in v2.3",
                                enum: ["6", "10"],
                                default: "6",
                            },
                            resolution: {
                                type: "string",
                                description: "Resolution of video (standard quality only). v02: 512P/768P, v2.3: 768P/1080P",
                                enum: ["512P", "768P", "1080P"],
                                default: "768P",
                            },
                            promptOptimizer: {
                                type: "boolean",
                                description: "Whether to use the model's prompt optimizer for better results",
                                default: true,
                            },
                            callBackUrl: {
                                type: "string",
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            input_urls: {
                                type: "array",
                                description: "Reference images for image-to-image mode (1-8 URLs). Omit for text-to-image mode.",
                                items: { type: "string", format: "uri" },
                                minItems: 1,
                                maxItems: 8,
                            },
                            aspect_ratio: {
                                type: "string",
                                description: "Aspect ratio for the generated image. 'auto' only valid with input_urls.",
                                enum: [
                                    "1:1",
                                    "4:3",
                                    "3:4",
                            ...
                            resolution: {
                                type: "string",
                                description: "Output resolution. Pro: 1K (~$0.025), 2K (~$0.035). Flex: 1K (~$0.07), 2K (~$0.12).",
                                enum: ["1K", "2K"],
                                default: "1K",
                            },
                            model_type: {
                                type: "string",
                                description: "Model variant: 'pro' for fast reliable results, 'flex' for more control and fine-tuning.",
                                enum: ["pro", "flex"],
                                default: "pro",
                            },
                            callBackUrl: {
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            video_url: {
                                type: "string",
                                description: "URL of the reference video (MP4, QUICKTIME, X-MATROSKA, max 10MB, max 30 seconds)",
                                format: "uri",
                            },
                            image_url: {
                                type: "string",
                                description: "URL of the character image (JPEG, PNG, WEBP, max 10MB). Will be resized and center-cropped to match video aspect ratio.",
                                format: "uri",
                            },
                            mode: {
                                type: "string",
                                description: "Animation mode: 'animate' transfers motion/expressions from video to image, 'replace' swaps the character in video with the image",
                                enum: ["animate", "replace"],
                                default: "animate",
                            },
                            resolution: {
                                type: "string",
                                description: "Output resolution: 480p (~$0.03/sec), 580p (~$0.0475/sec), 720p (~$0.0625/sec)",
                                enum: ["480p", "580p", "720p"],
                                default: "480p",
                            },
                            callBackUrl: {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            try {
                ...
                if (!this.enabledTools.has(name)) {
                    throw new McpError(ErrorCode.InvalidRequest, `Tool '${name}' is not enabled. This tool has been disabled by server configuration. ` +
        ...
        this.server.setRequestHandler(GetPromptRequestSchema, async (request) => {
            ...
            switch (name) {
                ...
                default:
                    throw new McpError(ErrorCode.InternalError, `Prompt not found: ${name}`);
            }
        });
    }
    async handleNanoBananaImage(args) {
        try {
        ...
        try {
            ...
            try {
                ...
                if (apiResponse?.data) {
                    ...
                    else if (localTask?.api_type === "elevenlabs-tts" ||
    ...
    async handleElevenLabsTTS(args) {
        try {
    ...
    async handleElevenLabsSoundEffects(args) {
        try {
    ...
    async handleByteDanceSeedanceVideo(args) {
        try {
    ...
    async handleGrokImagine(args) {
        try {
            ...
            if (response.code === 200 && response.data?.taskId) {
                ...
                const detectedMode = request.generation_mode ||
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                pricing: detectedMode === "text-to-image"
                                    ? "~$0.02 per image"
            ...
            if (error instanceof z.ZodError) {
                return this.formatError("grok_imagine", error, {
                    ...
                    generation_mode: "Explicit mode: text-to-image, text-to-video, image-to-video, upscale",
                });
            }
            return this.formatError("grok_imagine", error, {
                prompt: "Text prompt (required for text modes, optional for image-to-video)",
    ...
    async handleInfiniTalkLipSync(args) {
        try {
    ...
    async handleMidjourneyGenerate(args) {
        try {
            ...
            if (response.code === 200 && response.data?.taskId) {
                ...
                const isVideoMode = request.motion ||
    ...
    async handleFluxKontextImage(args) {
        try {
    ...
    async handleIdeogramReframe(args) {
        try {
        ...
        try {
            ...
            if (error instanceof z.ZodError) {
                return this.formatError("hailuo_video", error, {
                    ...
                    endImageUrl: "Optional: end frame image URL for image-to-video (requires imageUrl)",
                    version: 'Optional: model version "02" (default) or "2.3" (enhanced motion)',
                    quality: 'Optional: quality level "standard" (default) or "pro"',
                    duration: 'Optional: video duration "6" (default) or "10" (10s not supported with 1080P in v2.3)',
                    resolution: 'Optional: resolution "512P"/"768P" for v02, "768P"/"1080P" for v2.3',
        ...
        try {
            ...
            if (error instanceof z.ZodError) {
                return this.formatError("flux2_image", error, {
                    ...
                    resolution: "Optional: output resolution (1K or 2K). Default: 1K. Pro: 1K~$0.025, 2K~$0.035. Flex: 1K~$0.07, 2K~$0.12.",
        ...
        try {
            ...
            if (error instanceof z.ZodError) {
                return this.formatError("wan_animate", error, {
                    ...
                    resolution: 'Optional: "480p" (default, ~$0.03/sec), "580p" (~$0.0475/sec), or "720p" (~$0.0625/sec)',
                    callBackUrl: "Optional: callback URL for notifications (uses KIE_AI_CALLBACK_URL env var if not provided)",
                });
            }
            return this.formatError("wan_animate", error, {
    ...
    async getUsageStats() {
        try {
            ...
            const recentTasks = allTasks.filter((task) => new Date(task.created_at) > oneDayAgo);
            return JSON.stringify({
                timestamp: new Date().toISOString(),
                ...
                most_used_model: Object.keys(usageByType).reduce((a, b) => (usageByType[a] > usageByType[b] ? a : b), ""),
            }, null, 2);
        }
        catch (error) {
            return JSON.stringify({
...
`;
    ...
    async getAgentInstructions(agentName) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to load agent instructions for ${agentName}: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    async getModelDocumentation(modelKey) {
        const fs = await import("fs/promises");
        const path = await import("path");
        const { fileURLToPath } = await import("url");
        const __filename = fileURLToPath(import.meta.url);
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to load model documentation for ${modelKey}: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    async run() {
